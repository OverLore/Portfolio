export const projects = [
	{
		id: 'kunai-fu',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Indisponible pour le moment...',
		buttonTitleIcon: 'ri-download-2-line',
		link: '',
		name: 'Kunai-Fu',
		description:
			'Devenez un ninja et embrochez vos amis dans ce jeu multijoueur dynamique. Kunai-Fu est un jeu multijoueur en ligne jouable sur PC realisé sur Unreal Engine 5 par une équipe de 8 personnes.',
		image: 'projects/kunai-fu/preview.webp',
		stack: ['unreal', 'cpp', 'photoshop'],
		screenshots: [
			{ type: 'video', link: '7eBQQ-Wvbis' },
			{ type: 'image', link: 'incoming.webp' },
			{ type: 'image', link: 'kunai-fu/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'PC (Multijoueur)' },
			{ title: 'Genre', value: 'Combat / Party' },
			{ title: 'Durée de production', value: '8 semaines' },
			{ title: 'Langues', value: 'Anglais' }
		],
		credits: [
			{
				name: 'Luc Arnould',
				linkedin: 'https://www.linkedin.com/in/luc-arnould/',
				role: 'Programmation, Voix'
			},
			{
				name: 'Evan Batsik',
				linkedin: 'https://www.linkedin.com/in/evanbatsik/',
				role: 'Programmation, Son'
			},
			{
				name: 'Mathis Lagardère',
				linkedin: 'https://www.linkedin.com/in/mathis-lagardere/',
				role: 'Programmation'
			},
			{
				name: 'Denis Lochet',
				linkedin: 'https://www.linkedin.com/in/denislochet/',
				role: 'Programmation'
			},
			{
				name: 'Alan Paradis',
				linkedin: 'https://www.linkedin.com/in/alan-paradis/',
				role: 'Programmation'
			},
			{
				name: 'Laëtitia Simon',
				linkedin: 'https://www.linkedin.com/in/laetitia-simon1/',
				role: 'Infographie'
			},
			{
				name: 'Tanguy Desgranges',
				linkedin: 'https://www.linkedin.com/in/tanguymonkey/',
				role: 'Infographie, Son'
			},
			{
				name: 'Romain Espinosa',
				linkedin: 'https://www.linkedin.com/in/romain-espinosa/',
				role: 'Infographie'
			}
		],
		opacity: 0.85
	},
	{
		id: 'ligne-13',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Télécharger sur itch.io',
		buttonTitleIcon: 'ri-download-2-line',
		link: 'https://deusgames.itch.io/ligne-13',
		name: 'Ligne 13',
		description:
			"Vous vous réveillez dans un métro français après l'extinction de l'humanité. Votre seul espoir est de parcourir les ruines de la ligne 13 et d'atteindre le terminus. Ligne 13 est un jeu de survie solo jouable sur PC réalisé sur Unity par une équipe de 13 personnes.",
		image: 'projects/ligne13/preview.webp',
		stack: ['unity', 'csharp', 'photoshop', 'substance', 'blender', 'wwise', 'git'],
		screenshots: [
			{ type: 'video', link: '9ql-30FD55I' },
			{ type: 'image', link: 'ligne13/1.webp' },
			{ type: 'image', link: 'ligne13/2.webp' },
			{ type: 'image', link: 'ligne13/3.webp' },
			{ type: 'image', link: 'ligne13/4.webp' },
			{ type: 'image', link: 'ligne13/5.webp' },
			{ type: 'image', link: 'ligne13/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'PC' },
			{ title: 'Genre', value: 'Survie / FPS' },
			{ title: 'Durée de production', value: '8 semaines' },
			{ title: 'Langues', value: 'Anglais / Français' }
		],
		credits: [
			{
				name: 'Luc Arnould',
				linkedin: 'https://www.linkedin.com/in/luc-arnould/',
				role: 'Programmation'
			},
			{
				name: 'Evan Batsik',
				linkedin: 'https://www.linkedin.com/in/evanbatsik/',
				role: 'Programmation, Son'
			},
			{
				name: 'Nathan Falourd',
				linkedin: 'https://www.linkedin.com/in/nathan-falourd-09ab70255/',
				role: 'Programmation'
			},
			{
				name: 'Rémy Guillo',
				linkedin: 'https://www.linkedin.com/in/remy-guillo-20b830240/',
				role: 'Programmation'
			},
			{
				name: 'Mathis Lagardère',
				linkedin: 'https://www.linkedin.com/in/mathis-lagardere/',
				role: 'Programmation'
			},
			{
				name: 'Denis Lochet',
				linkedin: 'https://www.linkedin.com/in/denislochet/',
				role: 'Programmation'
			},
			{
				name: 'Alan Paradis',
				linkedin: 'https://www.linkedin.com/in/alan-paradis/',
				role: 'Programmation'
			},
			{
				name: 'Jules Baud',
				linkedin: 'https://www.linkedin.com/in/julesbaud/',
				role: 'Infographie'
			},
			{
				name: 'Ludivine Brun',
				linkedin: 'https://www.linkedin.com/in/ludivine-brun/',
				role: 'Infographie'
			},
			{
				name: 'Christopher Chaplain-Dumontier',
				linkedin: 'https://www.linkedin.com/in/christopher-cd/',
				role: 'Infographie'
			},
			{
				name: 'Tanguy Desgranges',
				linkedin: 'https://www.linkedin.com/in/tanguymonkey/',
				role: 'Infographie, Son'
			},
			{
				name: 'Loreena (Kitsu) Faucher',
				linkedin: 'https://www.linkedin.com/in/loreena-faucher-74a917194/',
				role: 'Infographie, Son'
			},
			{
				name: 'Nathan Laguna',
				linkedin: 'https://www.linkedin.com/in/nathanlaguna/',
				role: 'Infographie'
			}
		],
		opacity: 0.5
	},
	{
		id: 'kurage',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Télécharger le jeu',
		buttonTitleIcon: 'ri-download-2-line',
		link: 'https://www.mediafire.com/file/2mjs1q022awazch/KurageFinal3.apk/file',
		name: 'Kurage',
		description:
			'Incarnez une méduse et sauvez les océans de vagues de poissons aux projets sombres. Kurage est un jeu android solo réalisé sur Unity par une équipe de 9 personnes.',
		image: 'projects/kurage/preview.webp',
		stack: ['unity', 'csharp', 'photoshop', 'git'],
		screenshots: [
			{ type: 'image', link: 'kurage/2.webp' },
			{ type: 'image', link: 'kurage/3.webp' },
			{ type: 'image', link: 'kurage/4.webp' },
			{ type: 'image', link: 'kurage/5.webp' },
			{ type: 'image', link: 'kurage/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'Android' },
			{ title: 'Genre', value: "Shoot'em Up" },
			{ title: 'Durée de production', value: '4 semaines' },
			{ title: 'Langues', value: 'Français' }
		],
		credits: [
			{
				name: 'Luc Arnould',
				linkedin: 'https://www.linkedin.com/in/luc-arnould/',
				role: 'Programmation, Son'
			},
			{
				name: 'Evan Batsik',
				linkedin: 'https://www.linkedin.com/in/evanbatsik/',
				role: 'Programmation, Son'
			},
			{
				name: 'Mathis Lagardère',
				linkedin: 'https://www.linkedin.com/in/mathis-lagardere/',
				role: 'Programmation'
			},
			{
				name: 'Denis Lochet',
				linkedin: 'https://www.linkedin.com/in/denislochet/',
				role: 'Programmation'
			},
			{
				name: 'Alan Paradis',
				linkedin: 'https://www.linkedin.com/in/alan-paradis/',
				role: 'Programmation'
			},
			{
				name: 'Jules Baud',
				linkedin: 'https://www.linkedin.com/in/julesbaud/',
				role: 'Infographie'
			},
			{
				name: 'Tanguy Desgranges',
				linkedin: 'https://www.linkedin.com/in/tanguymonkey/',
				role: 'Infographie, Son'
			},
			{
				name: 'Loreena (Kitsu) Faucher',
				linkedin: 'https://www.linkedin.com/in/loreena-faucher-74a917194/',
				role: 'Infographie'
			},
			{
				name: 'Marie Berthet',
				linkedin: 'https://www.linkedin.com/in/marieberthet/',
				role: 'Infographie'
			}
		],
		opacity: 0.85
	},
	{
		id: 'naukar',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Télécharger le jeu',
		buttonTitleIcon: 'ri-download-2-line',
		link: 'https://deus-games-studio.itch.io/naukar',
		name: 'Naukar',
		description:
			'Incarnez Naukar, un lapin mage venu sauver son maître des tentacules de la vile Veyshia. Naukar est un roguelite jouable en solo réalisé avec la librairie C-SFML par un groupe de 9 personnes.',
		image: 'projects/naukar/preview.webp',
		stack: ['sfml', 'c', 'photoshop'],
		screenshots: [
			{ type: 'video', link: 'igblL_L4au0' },
			{ type: 'image', link: 'naukar/0.webp' },
			{ type: 'image', link: 'naukar/1.webp' },
			{ type: 'image', link: 'naukar/1-2.webp' },
			{ type: 'image', link: 'naukar/1-3.webp' },
			{ type: 'image', link: 'naukar/2.webp' },
			{ type: 'image', link: 'naukar/3.webp' },
			{ type: 'image', link: 'naukar/4.webp' },
			{ type: 'image', link: 'naukar/4-2.webp' },
			{ type: 'image', link: 'naukar/5.webp' },
			{ type: 'image', link: 'naukar/6.webp' },
			{ type: 'image', link: 'naukar/7.webp' },
			{ type: 'image', link: 'naukar/8.webp' },
			{ type: 'image', link: 'naukar/10.webp' },
			{ type: 'image', link: 'naukar/11.webp' },
			{ type: 'image', link: 'naukar/12.webp' },
			{ type: 'image', link: 'naukar/14.webp' },
			{ type: 'image', link: 'naukar/15.webp' },
			{ type: 'image', link: 'naukar/16.webp' },
			{ type: 'image', link: 'naukar/17.webp' },
			{ type: 'image', link: 'naukar/17-2.webp' },
			{ type: 'image', link: 'naukar/19.webp' },
			{ type: 'image', link: 'naukar/20.webp' },
			{ type: 'image', link: 'naukar/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'PC' },
			{ title: 'Genre', value: 'Roguelite' },
			{ title: 'Durée de production', value: '8 semaines' },
			{ title: 'Langues', value: 'Français' }
		],
		credits: [
			{
				name: 'Luc Arnould',
				linkedin: 'https://www.linkedin.com/in/luc-arnould/',
				role: 'Programmation, Son'
			},
			{
				name: 'Evan Batsik',
				linkedin: 'https://www.linkedin.com/in/evanbatsik/',
				role: 'Programmation, Son'
			},
			{
				name: 'Mathis Lagardère',
				linkedin: 'https://www.linkedin.com/in/mathis-lagardere/',
				role: 'Programmation'
			},
			{
				name: 'Denis Lochet',
				linkedin: 'https://www.linkedin.com/in/denislochet/',
				role: 'Programmation'
			},
			{
				name: 'Alan Paradis',
				linkedin: 'https://www.linkedin.com/in/alan-paradis/',
				role: 'Programmation'
			},
			{
				name: 'Jules Baud',
				linkedin: 'https://www.linkedin.com/in/julesbaud/',
				role: 'Infographie'
			},
			{
				name: 'Marie Berthet',
				linkedin: 'https://www.linkedin.com/in/marieberthet/',
				role: 'Infographie, Musique'
			},
			{
				name: 'Tanguy Desgranges',
				linkedin: 'https://www.linkedin.com/in/tanguymonkey/',
				role: 'Infographie, Son'
			},
			{
				name: 'Loreena (Kitsu) Faucher',
				linkedin: 'https://www.linkedin.com/in/loreena-faucher-74a917194/',
				role: 'Infographie, Son'
			},
			{
				name: 'Nathan Laguna',
				linkedin: 'https://www.linkedin.com/in/nathanlaguna/',
				role: 'Infographie'
			}
		],
		opacity: 0.85
	},
	{
		id: 'vr-playground',
		type: 'game',
		button: 'download',
		buttonText: 'Voir le jeu',
		buttonTitle: 'Voir sur GitHub',
		buttonTitleIcon: 'ri-links-fill',
		link: 'https://github.com/OverLore/VR-Playground/',
		name: 'VR Playground',
		description:
			"VR Playground est un mini jeu de démo de technologie VR sur Unity. Intéragissez avec l'environnement ou bien jouer à Danger Ball, un clone minimaliste du jeu du même nom sorti sur Playstation VR World.",
		image: 'projects/vr-playground/preview.webp',
		stack: ['unity', 'csharp', 'blender', 'git'],
		screenshots: [
			{ type: 'image', link: 'vr-playground/preview.webp' },
			{ type: 'image', link: 'incoming.webp' }
		],
		opacity: 0.85
	},
	{
		id: 'under-pressure',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Indisponible pour le moment...',
		buttonTitleIcon: 'ri-download-2-line',
		link: '',
		name: 'Under Pressure',
		description:
			"Rejoignez une équipe de 4 personnages devant survivre 10 tours dans leur sous-marin. Under Pressure est un portage C-SFML d'un jeu de plateau réalisé par 9 personnes.",
		image: 'projects/under-pressure/preview.webp',
		stack: ['sfml', 'c', 'photoshop'],
		screenshots: [
			{ type: 'image', link: 'under-pressure/1.webp' },
			{ type: 'image', link: 'under-pressure/2.webp' },
			{ type: 'image', link: 'under-pressure/3.webp' },
			{ type: 'image', link: 'under-pressure/4.webp' },
			{ type: 'image', link: 'under-pressure/5.webp' },
			{ type: 'image', link: 'under-pressure/6.webp' },
			{ type: 'image', link: 'under-pressure/7.webp' },
			{ type: 'image', link: 'under-pressure/8.webp' },
			{ type: 'image', link: 'under-pressure/9.webp' },
			{ type: 'image', link: 'under-pressure/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'PC' },
			{ title: 'Genre', value: 'Jeu de plateau' },
			{ title: 'Durée de production', value: '2 semaines' },
			{ title: 'Langues', value: 'Anglais' }
		],
		credits: [
			{
				name: 'Luc Arnould',
				linkedin: 'https://www.linkedin.com/in/luc-arnould/',
				role: 'Programmation, Son'
			},
			{
				name: 'Evan Batsik',
				linkedin: 'https://www.linkedin.com/in/evanbatsik/',
				role: 'Programmation, Son'
			},
			{
				name: 'Mathis Lagardère',
				linkedin: 'https://www.linkedin.com/in/mathis-lagardere/',
				role: 'Programmation'
			},
			{
				name: 'Denis Lochet',
				linkedin: 'https://www.linkedin.com/in/denislochet/',
				role: 'Programmation'
			},
			{
				name: 'Alan Paradis',
				linkedin: 'https://www.linkedin.com/in/alan-paradis/',
				role: 'Programmation'
			},
			{
				name: 'Jules Baud',
				linkedin: 'https://www.linkedin.com/in/julesbaud/',
				role: 'Infographie'
			},
			{
				name: 'Tanguy Desgranges',
				linkedin: 'https://www.linkedin.com/in/tanguymonkey/',
				role: 'Infographie, Son'
			},
			{
				name: 'Loreena (Kitsu) Faucher',
				linkedin: 'https://www.linkedin.com/in/loreena-faucher-74a917194/',
				role: 'Infographie'
			},
			{
				name: 'Marie Berthet',
				linkedin: 'https://www.linkedin.com/in/marieberthet/',
				role: 'Infographie'
			}
		],
		opacity: 0.85
	},
	{
		id: 'cookie-maker',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Télécharger le jeu',
		buttonTitleIcon: 'ri-download-2-line',
		link: 'https://www.mediafire.com/file/kvq54tv9kco4wya/CookieMaker_ARNOULD.apk/file',
		name: 'Cookie Maker',
		description:
			'Cookie Maker est une maquette de jeu hyper-casual où vous devez créer le plus gros cookie possible. Cette maquette à été réalisée en 3 heures sur Unity.',
		image: 'projects/cookie-maker/preview.webp',
		stack: ['unity', 'csharp', 'photoshop', 'blender'],
		screenshots: [
			{ type: 'image', link: 'cookie-maker/1.webp' },
			{ type: 'image', link: 'cookie-maker/2.webp' },
			{ type: 'image', link: 'cookie-maker/3.webp' },
			{ type: 'image', link: 'cookie-maker/4.webp' },
			{ type: 'image', link: 'cookie-maker/5.webp' },
			{ type: 'image', link: 'cookie-maker/6.webp' },
			{ type: 'image', link: 'cookie-maker/7.webp' },
			{ type: 'image', link: 'cookie-maker/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'Android' },
			{ title: 'Genre', value: 'Hyper-Casual' },
			{ title: 'Durée de production', value: '3 heures' },
			{ title: 'Langues', value: 'Français' }
		],
		opacity: 0.85
	},
	{
		id: 'flappy-clone',
		type: 'game',
		button: 'download',
		buttonText: 'Télécharger',
		buttonTitle: 'Télécharger FlappyClone (.apk)',
		buttonTitleIcon: 'ri-download-2-line',
		link: 'https://download1078.mediafire.com/hp712sw5b3wgYzCfFWYNIo9rbBuCe8sKqcPY_eYBnpJEDnH6Bu-fUk6B7_KPfseiMwD1eaaPUdXsj98LZSlOIH1Wew1gkdbM1Q4cq_niWlni8XV9-KgfDh3cABjKOkDdb7g69Rx9jSLXElGksWPE83jBUsfTQybKR5Iwx9bWH95R0Q/z9ku3cbgmttgpmk/Flappy+bird+Arnould.apk',
		name: 'Flappy Clone',
		description:
			'Flappy Clone est une copie du jeu iconique Flappy Bird jouable en solo sur android réalisée avec la libraire Löve2D.',
		image: 'projects/flappy-clone/preview.webp',
		stack: ['love2d', 'lua', 'photoshop'],
		screenshots: [
			{ type: 'image', link: 'flappy-clone/1.webp' },
			{ type: 'image', link: 'flappy-clone/2.webp' },
			{ type: 'image', link: 'flappy-clone/3.webp' },
			{ type: 'image', link: 'flappy-clone/4.webp' },
			{ type: 'image', link: 'flappy-clone/5.webp' },
			{ type: 'image', link: 'flappy-clone/6.webp' },
			{ type: 'image', link: 'flappy-clone/preview.webp' }
		],
		infos: [
			{ title: 'Plateforme', value: 'Android' },
			{ title: 'Genre', value: 'Casual' },
			{ title: 'Durée de production', value: '2 semaines' },
			{ title: 'Langues', value: 'Anglais' }
		],
		opacity: 0.85
	},
	{
		id: 'portfolio',
		type: 'site',
		button: 'view',
		buttonText: 'Accéder',
		buttonTitle: 'Consulter le site web',
		buttonTitleIcon: 'ri-links-fill',
		link: 'https://www.lucarnould.fr/',
		name: 'Portfolio',
		description:
			'Le portfolio sur lequel vous vous trouvez tout simplement. Ce portfolio a été designé sur Figma puis réalisé avec SvelteKit et propulsé par Vite.',
		image: 'projects/portfolio/preview.webp',
		stack: ['svelte', 'vite', 'node', 'figma'],
		screenshots: [
			{ type: 'image', link: 'portfolio/1.webp' },
			{ type: 'image', link: 'portfolio/2.webp' },
			{ type: 'image', link: 'portfolio/3.webp' },
			{ type: 'image', link: 'portfolio/4.webp' },
			{ type: 'image', link: 'portfolio/5.webp' },
			{ type: 'image', link: 'portfolio/6.webp' },
			{ type: 'image', link: 'portfolio/preview.webp' }
		],
		opacity: 0.85
	},
	{
		id: 'burger-evasion',
		type: 'site',
		button: 'view',
		buttonText: 'Accéder',
		buttonTitle: 'Consulter le site web',
		buttonTitleIcon: 'ri-links-fill',
		link: 'https://burgers.lucarnould.fr/',
		name: 'Burger Evasion',
		description:
			"Site d'un restaurant fictif commercialisant des burgers. Ce portfolio a été designé sur Figma puis réalisé avec SvelteKit et propulsé par Vite.",
		image: 'projects/burgers/preview.webp',
		stack: ['svelte', 'vite', 'node', 'figma'],
		screenshots: [
			{ type: 'image', link: 'burgers/1.webp' },
			{ type: 'image', link: 'burgers/2.webp' },
			{ type: 'image', link: 'burgers/3.webp' },
			{ type: 'image', link: 'burgers/4.webp' },
			{ type: 'image', link: 'burgers/5.webp' },
			{ type: 'image', link: 'burgers/preview.webp' }
		],
		opacity: 0.85
	},
	{
		id: 'virtuos',
		type: 'software',
		button: 'none',
		buttonText: 'Indisponible',
		buttonTitle: 'Rien à télécharger ici...',
		buttonTitleIcon: '',
		link: '',
		name: 'Sous NDA',
		description:
			'Projets professionnels réalisés chez Virtuos pour leurs clients. Ces projets étants sous NDA, je ne peux pas les décrire. Seulement parler des stacks utilisées.',
		image: 'projects/virtuos/preview.webp',
		stack: [
			'blazor',
			'mysql',
			'wpf',
			'dotnet',
			'aspdotnet',
			'restapi',
			'csharp',
			'cpp',
			'js',
			'python',
			'superluminal',
			'postman',
			'azure',
			'p4'
		],
		screenshots: [{ type: 'image', link: 'virtuos/preview.webp' }],
		opacity: 0.85
	}
];
